...
class fpgrowthSuite extends FunSuite {
  ...
  class MyRegistrator extends KryoRegistrator {
    override def registerClasses(kryo: Kryo) {
      //kryo.addDefaultSerializer(classOf[Node], classOf[NodeSerializer])
      kryo.register(classOf[FPTree])
      kryo.register(classOf[Node], new NodeSerializer())
      //kryo.register(classOf[(Stack[Int],Node)])
    }
  }
  ...
  def conf = new SparkConf()
    ...
    .set("spark.kryo.registrator", "fptree.MyRegistrator")
  ...
  test("node serializer test"){
    ...
    kryo.writeObject(output, outputNode)
    output.close()
    
    val input = new Input(new FileInputStream("file.bin"))
    val inputNode = kryo.readObject(input, classOf[Node])
    info("tree \n" + inputNode + "\n end.")
    input.close()
  }
