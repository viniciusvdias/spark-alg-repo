...
class PageRank (
      ...
      numPartitions: Int,
      numIterations: Int,
      inputType: String = PageRank.INPUT_EDGES)
  extends Logging {
...
class SparkPageRank (inputFile: String,
    ...
    numIterations: Int,
    optHelper: OptHelper = new OptHelper) extends Logging {
  ...
  def run {
    ...
    for (i <- 1 to numIterations) {
      ...
      val _ranks = ranks
      val contribs = optHelper.adaptRDD("adaptive-point-contributions",
          links.join(_ranks, numPartitions(1)),
          links
        ).values.flatMap{ case (urls, rank) =>
            val size = urls.size
            urls.map(url => (url, rank / size))
          }
      ranks = optHelper.adaptRDD("adaptive-point-newrank", 
        contribs.reduceByKey(_ + _, numPartitions(2)),
        contribs).
  ...
  def main(args: Array[String]) {
    args(3) match {
      case PageRank.SPARK_PAGERANK =>
        ...
        // args(0): input file
        val optHelper = if (args.size >= 6 && args(4) == "--optPlan")
          OptHelper(args(5))
        else new OptHelper
        val numPartitions = (args(1) split ",").map (_.toInt)
        new SparkPageRank (args(0), numPartitions, args(2).toInt, optHelper).run
      case _ =>
