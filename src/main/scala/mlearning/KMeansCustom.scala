...
package org.apache.spark.mllib.clustering.custom
...
class KMeans private (
   ...
   */
  @Since("0.8.0")
  def run(data: RDD[Vector], maybeIterationPartitions: Int = -1): KMeansModel = {
    ...
    val iterationPartitions = maybeIterationPartitions match {
      case p if p < 1 => data.partitions.size
      case p => p
    }
    ...
    val model = runAlgorithm(zippedData, iterationPartitions)
    norms.unpersist()
   ...
   */
  private def runAlgorithm(data: RDD[VectorWithNorm], iterationPartitions: Int): KMeansModel = {
    ...
    while (iteration < maxIterations && !activeRuns.isEmpty) {
      ...
      val totalContribs = data.mapPartitions { points =>
        ...
        contribs.iterator
      }.reduceByKey(mergeContribs _, iterationPartitions).collectAsMap()
...
object KMeans {
  ...
  // custom kmeans, for test purposes
  def train(
      data: RDD[Vector],
      numPartitions: Int,
      k: Int,
